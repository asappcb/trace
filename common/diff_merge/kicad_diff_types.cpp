                              ...
                              const EDA_IU_SCALE& aScale )
{
    ...
    std::function<void( const ITEM_CHANGE&, int )> writeChange = [&]( const ITEM_CHANGE& aChange, int aDepth )
    {
        ...
        {
            ss << propIndent << p.name.ToStdString() << ": " << p.before.ToDisplayString( aUnits, aScale ).ToStdString()
