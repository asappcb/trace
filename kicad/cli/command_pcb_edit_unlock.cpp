...
#include "command_pcb_edit_unlock.h"
...
CLI::PCB_EDIT_UNLOCK_COMMAND::PCB_EDIT_UNLOCK_COMMAND() :
        COMMAND( "unlock" )
{
    addCommonArgs( true, true, IO_TYPE::FILE, IO_TYPE::FILE );

    m_argParser.add_description(
            UTF8STDSTR( _( "Unlock a footprint (--ref) or a net's tracks and vias (--net) and write the board back "
...
int CLI::PCB_EDIT_UNLOCK_COMMAND::doPerform( KIWAY& aKiway )
{
