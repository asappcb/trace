...
/**
 * The pad gate/pin-swap equivalence group (unit + position index, transported from the schematic
 * symbol) must round-trip through the board s-expr. A pad with no group must not emit the token and
 * must load back as not-eligible.
 */
BOOST_AUTO_TEST_CASE( PadPinSwapGroupRoundTrips )
{
    std::filesystem::path tmpPath = std::filesystem::temp_directory_path()
                                    / "pad_pin_swap_group.kicad_pcb";

    {
        ...
        FOOTPRINT* fp = new FOOTPRINT( board.get() );
        board->Add( fp );
        fp->SetFPID( LIB_ID( wxT( "scratch" ), wxT( "swap_fp" ) ) );

        // Two pads that are the same position (index 0) of two interchangeable units (gates 1, 2).
        PAD* gate1 = new PAD( fp );
        gate1->SetNumber( wxT( "1" ) );
        gate1->SetShape( PADSTACK::ALL_LAYERS, PAD_SHAPE::CIRCLE );
        gate1->SetSize( PADSTACK::ALL_LAYERS, VECTOR2I( pcbIUScale.mmToIU( 1 ), pcbIUScale.mmToIU( 1 ) ) );
        gate1->SetPinSwapGroup( 1, 0 );
        fp->Add( gate1 );

        PAD* gate2 = new PAD( fp );
        gate2->SetNumber( wxT( "2" ) );
        gate2->SetShape( PADSTACK::ALL_LAYERS, PAD_SHAPE::CIRCLE );
        gate2->SetSize( PADSTACK::ALL_LAYERS, VECTOR2I( pcbIUScale.mmToIU( 1 ), pcbIUScale.mmToIU( 1 ) ) );
        gate2->SetPinSwapGroup( 2, 0 );
        fp->Add( gate2 );

        // A pad from a locked/single-unit symbol: not swap-eligible, must emit no token.
        PAD* plain = new PAD( fp );
        plain->SetNumber( wxT( "3" ) );
        plain->SetShape( PADSTACK::ALL_LAYERS, PAD_SHAPE::CIRCLE );
        plain->SetSize( PADSTACK::ALL_LAYERS, VECTOR2I( pcbIUScale.mmToIU( 1 ), pcbIUScale.mmToIU( 1 ) ) );
        fp->Add( plain );

        BOOST_REQUIRE( !plain->IsPinSwapEligible() );

        PCB_IO_KICAD_SEXPR writer;
        writer.SaveBoard( tmpPath.string(), board.get() );
    }

    // The eligible pads emit the token; the ineligible one does not.
    {
        ...
        BOOST_CHECK( contents.find( "(pin_swap_group 1 0)" ) != std::string::npos );
        BOOST_CHECK( contents.find( "(pin_swap_group 2 0)" ) != std::string::npos );

        // Exactly two occurrences (the two gates); the plain pad must not add a third.
        size_t count = 0;
        for( size_t pos = contents.find( "(pin_swap_group" );
             pos != std::string::npos;
             pos = contents.find( "(pin_swap_group", pos + 1 ) )
        {
            ++count;
        }
    ...
    // And they load back with the same unit/index; the plain pad stays not-eligible.
    {
        std::unique_ptr<BOARD> readBoard = std::make_unique<BOARD>();
        PCB_IO_KICAD_SEXPR     reader;
        reader.LoadBoard( tmpPath.string(), readBoard.get() );

        BOOST_REQUIRE_EQUAL( readBoard->Footprints().size(), 1u );
        FOOTPRINT* fp = readBoard->Footprints().front();

        PAD* gate1 = fp->FindPadByNumber( wxT( "1" ) );
        PAD* gate2 = fp->FindPadByNumber( wxT( "2" ) );
        PAD* plain = fp->FindPadByNumber( wxT( "3" ) );

        BOOST_REQUIRE( gate1 && gate2 && plain );

        BOOST_CHECK( gate1->IsPinSwapEligible() );
        BOOST_CHECK_EQUAL( gate1->GetPinSwapUnit(), 1 );
        BOOST_CHECK_EQUAL( gate1->GetPinSwapIndex(), 0 );

        BOOST_CHECK( gate2->IsPinSwapEligible() );
        BOOST_CHECK_EQUAL( gate2->GetPinSwapUnit(), 2 );
        BOOST_CHECK_EQUAL( gate2->GetPinSwapIndex(), 0 );

        BOOST_CHECK( !plain->IsPinSwapEligible() );
        BOOST_CHECK_EQUAL( plain->GetPinSwapIndex(), -1 );
    }
